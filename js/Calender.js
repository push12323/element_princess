...
function createCalendar() {
	...
	calenderContainer = new createjs.Container(); 
	calenderContainer.x = 91;
	calenderContainer.y = 173;
	stage.addChild(calenderContainer);
	
	var monthText = new createjs.Text();
	monthText.text = showDate.getFullMonthName(monthNum);
	monthText.font = "bold 19px Times New Roman"
	monthText.color = "#ffffff"
	monthText.x = 0;
	monthText.y = -35;
	calenderContainer.addChild(monthText);
	
	var shape1 = new createjs.Shape();
	shape1.graphics.beginFill("#fff");
	shape1.graphics.moveTo(100,-35);
	shape1.graphics.lineTo(110,-35);
	shape1.graphics.lineTo(105,-25);
	shape1.graphics.lineTo(100,-35);
	shape1.graphics.endFill();
	shape1.scaleY = -1;
	shape1.x = 0;
	shape1.y = -60;
	calenderContainer.addChild(shape1);
	shape1.addEventListener("click", shapeClickHandler1);
	shape1.addEventListener("mouseover", shapeOverHandler1);
	
	var shape2 = new createjs.Shape();
	shape2.graphics.beginFill("#fff");
	shape2.graphics.moveTo(100,-20);
	shape2.graphics.lineTo(110,-20);
	shape2.graphics.lineTo(105,-10);
	shape2.graphics.lineTo(100,-20);
	shape2.graphics.endFill();
	calenderContainer.addChild(shape2);
	shape2.addEventListener("click", shapeClickHandler2);
	shape2.addEventListener("mouseover", shapeOverHandler2);
	
	var yearText = new createjs.Text();
	yearText.text = yearNum;
	yearText.font = "bold 19px Times New Roman"
	yearText.color = "#ffffff"
	yearText.x = 280;
	yearText.y = -35;
	calenderContainer.addChild(yearText);
	
	var shape3 = new createjs.Shape();
	shape3.graphics.beginFill("#fff");
	shape3.graphics.moveTo(0,-35);
	shape3.graphics.lineTo(10,-35);
	shape3.graphics.lineTo(5,-25);
	shape3.graphics.lineTo(0,-35);
	shape3.graphics.endFill();
	shape3.scaleY = -1;
	shape3.x = 330;
	shape3.y = -60;
	calenderContainer.addChild(shape3);
	shape3.addEventListener("click", shapeClickHandler3);
	shape3.addEventListener("mouseover", shapeOverHandler3);
	
	var shape4 = new createjs.Shape();
	shape4.graphics.beginFill("#fff");
	shape4.graphics.moveTo(0,-20);
	shape4.graphics.lineTo(10,-20);
	shape4.graphics.lineTo(5,-10);
	shape4.graphics.lineTo(0,-20);
	shape4.graphics.endFill();
	shape4.x = 330;
	shape4.y = 0;
	calenderContainer.addChild(shape4);
	shape4.addEventListener("click", shapeClickHandler4);
	shape4.addEventListener("mouseover", shapeOverHandler4);
	...
	for (var i = 1; i <= 6; i++) {
		...
		var shape = new createjs.Shape();
 		shape.graphics.beginStroke("#000000").drawRect(0, (33*(i)), 350, shapeHeight);
		//calenderContainer.addChild(shape);
		for (var x = 0; x <= 6; x++) {
			...
			if ((daycounter > numbDays) || (daycounter < 1)) {
				...
				var shape = new createjs.Shape();
				shape.graphics.beginStroke("#00ff00").drawRect((shapeWidth*(x)), (shapeHeight*(i)), shapeWidth, shapeHeight);
				//calenderContainer.addChild(shape);
				var dateText = new createjs.Text();
				dateText.text = "";
				dateText.font = "bold 14px Times New Roman"
				dateText.color = "#000"
				dateText.x = (shapeWidth*(x));
				dateText.y = (shapeHeight*(i));
				calenderContainer.addChild(dateText);
			}
			else 
			{
				/*if ((todaysDate == daycounter) && (todaysMonth == monthNum)) {
					var shape = new createjs.Shape();
					shape.graphics.beginStroke("#0000FF").drawRect((shapeWidth*(x)), (shapeHeight*(i)), shapeWidth, shapeHeight);
					//calenderContainer.addChild(shape);
					
					var dateText = new createjs.Text();
					dateText.text = daycounter;
					dateText.font = "bold 14px Times New Roman"
				...
				if(checkevents(daycounter,monthNum,yearNum,i,x)){
					...
					var shape = new createjs.Shape();
					shape.graphics.beginStroke("#FF0000").drawRect((shapeWidth*(x)), (shapeHeight*(i)), shapeWidth, shapeHeight);
					//calenderContainer.addChild(shape);
					
					var calenderPrinces = new createjs.Bitmap("assets/calender_princes" + (calDayPrinces+1) +".png");
					
					calenderPrinces.x = shapeWidth*(x);
					calenderPrinces.y = shapeHeight*(i);
					calenderPrinces.id = calDayPrinces;
					calenderContainer.addChild(calenderPrinces);
					if(x > 4)
					{
						calenderPrinces.x = shapeWidth*(x)-(8-x);
					}
					
					calenderPrinces.addEventListener("click", calenderPrincesClick);
					calenderPrinces.addEventListener("mouseover", calenderPrincesOver);
					...
					var shape = new createjs.Shape();
					shape.graphics.beginStroke("#000").drawRect((shapeWidth*(x)), (shapeHeight*(i)), shapeWidth, shapeHeight);
					//calenderContainer.addChild(shape);
					...
					switch(monthNum)
					{
						case 1: case 3: case 5: case 7: case 8: case 10: case 12:
							...
							if(daycounter <= 31)
							{
								...
								dateText.font = "bold 14px Times New Roman"
								dateText.color = "#000"
								dateText.x = (shapeWidth*(x)+25);
								dateText.y = (shapeHeight*(i)+8);
								dateText.lineWidth = shapeWidth;
								calenderContainer.addChild(dateText);
							}
						break;
...
function calenderPrincesClick(event)
{
	playSound("button_click");
	console.log(event.target.id);
	showCalenderPrinces(event.target.id);
}


function shapeClickHandler1(event)
{
	//playSound("button_click");
	stage.removeChild(calenderContainer);
	changedate("nextmo");
	stage.update();
}

function shapeClickHandler2(event)
{
	//playSound("button_click");
	stage.removeChild(calenderContainer);
	changedate("prevmo");
...
function shapeClickHandler3(event)
{
	//playSound("button_click");
	stage.removeChild(calenderContainer);
	changedate("nextyr");
...
function shapeClickHandler4(event)
{
	//playSound("button_click");
	stage.removeChild(calenderContainer);
	changedate("prevyr");
...
function calenderPrincesOver(event)
{
	playSound("button_over");
}


function shapeOverHandler1(event)
{
	//playSound("button_over");
}

function shapeOverHandler2(event)
{
	//playSound("button_over");
}

function shapeOverHandler3(event)
{
	//playSound("button_over");
}

function shapeOverHandler4(event)
{
	//playSound("button_over");
}
...
var floater = 0;
var showDate = new ShowDate();
	for (var i = 0; i < playerPrinces.length; i++) {
		//console.log(playedDays[i].date + " : numevents : "+ day +" ================== "+playedDays[i].month + " : numevents : "+ showDate.getTodayMonth(today));
		if (playedDays[i].date == day && playedDays[i].month == showDate.getMonthName(monthNum) && playedDays[i].year == year) {
			//isPlayedToday = true;
			numevents++;
			calDayPrinces = playerPrinces[i];
			console.log("numevents : "+ calDayPrinces);	
			//console.log("playerPrinces  ==: "+ playerPrinces[calDayPrinces]);		
            return true;
        }
		/*if (playerPrinces[i][0] == "W") {
			if ((playerPrinces[i][2] == dayofweek)) numevents++;
		}
		else if (playerPrinces[i][0] == "Y") {
			if ((playerPrinces[i][2] == day) && (playerPrinces[i][1] == month)) numevents++;
		}
		else if (playerPrinces[i][0] == "F") {
			if ((playerPrinces[i][1] == 3) && (playerPrinces[i][2] == 0) && (playerPrinces[i][3] == 0) ) {
				easter(year);
				if (easterday == day && eastermonth == month) numevents++;
			} else {
				floater = floatingholiday(year,playerPrinces[i][1],playerPrinces[i][2],playerPrinces[i][3]);
				if ((month == 5) && (playerPrinces[i][1] == 5) && (playerPrinces[i][2] == 4) && (playerPrinces[i][3] == 2)) {
					if ((floater + 7 <= 31) && (day == floater + 7)) {
						numevents++;
					} else if ((floater + 7 > 31) && (day == floater)) numevents++;
				} else if ((playerPrinces[i][1] == month) && (floater == day)) numevents++;
			}
		}
		else if ((playerPrinces[i][2] == day) && (playerPrinces[i][1] == month) && (playerPrinces[i][3] == year)) {
			numevents++;
		}*/
	}
